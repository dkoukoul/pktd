...
func createNewSubServer(configRegistry lnrpc.SubServerConfigDispatcher) (lnrpc.SubServer, er.R) {
	...
	case config.FeeEstimator == nil:
		return nil, er.Errorf("FeeEstimator must be set to " +
			"create WalletKit RPC server")

	case config.Wallet == nil:
		return nil, er.Errorf("Wallet must be set to create " +
			"WalletKit RPC server")

	case config.KeyRing == nil:
		return nil, er.Errorf("KeyRing must be set to create " +
			"WalletKit RPC server")

	case config.Sweeper == nil:
		return nil, er.Errorf("Sweeper must be set to create " +
			"WalletKit RPC server")

	case config.Chain == nil:
		return nil, er.Errorf("Chain must be set to create " +
			"WalletKit RPC server")
	}
