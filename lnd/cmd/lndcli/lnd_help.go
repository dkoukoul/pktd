...
func getMasterHelp(pldServer string) er.R {
	...
	for _, line := range []string{
		...
		"which can be accessed by typing " + bright("pktctl help <the command>") + ".",
		"For example: " + bright("pktctl help wallet/unlock"),
		"",
		"In addition, each command corrisponds to an RPC endpoint which",
		"can be requested directly. So for example the command:",
		bright("pktctl meta/getinfo") + " is the same as API request:",
		bright(fmt.Sprintf("curl %s/api/v1/meta/getinfo", pldServer)),
		"",
		"Commands which require input arguments are done as POST messages",
		bright("pktctl wallet/unlock --wallet_passphrase=password"),
		...
		bright("pktctl help wallet/unlock") + " which is the same as the API request:",
