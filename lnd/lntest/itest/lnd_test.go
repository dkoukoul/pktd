...
func rpcPointToWirePoint(t *harnessTest, chanPoint *rpc_pb.ChannelPoint) wire.OutPoint {
...
func openChannelStream(ctx context.Context, t *harnessTest,
	...
	p lntest.OpenChannelParams) rpc_pb.Lightning_OpenChannelClient {
	...
	var chanOpenUpdate rpc_pb.Lightning_OpenChannelClient
...
func openChannelAndAssert(ctx context.Context, t *harnessTest,
	net *lntest.NetworkHarness, alice, bob *lntest.HarnessNode,
	p lntest.OpenChannelParams) *rpc_pb.ChannelPoint {
...
func closeChannelAndAssert(ctx context.Context, t *harnessTest,
	...
	fundingChanPoint *rpc_pb.ChannelPoint, force bool) *chainhash.Hash {
...
func closeChannelAndAssertType(ctx context.Context, t *harnessTest,
	net *lntest.NetworkHarness, node *lntest.HarnessNode,
	fundingChanPoint *rpc_pb.ChannelPoint, anchors, force bool) *chainhash.Hash {
...
func assertChannelClosed(ctx context.Context, t *harnessTest,
	net *lntest.NetworkHarness, node *lntest.HarnessNode,
	fundingChanPoint *rpc_pb.ChannelPoint, anchors bool,
	closeUpdates rpc_pb.Lightning_CloseChannelClient) *chainhash.Hash {
	...
	listChansRequest := &rpc_pb.ListChannelsRequest{}
	listChansResp, errr := node.ListChannels(ctxt, listChansRequest)
	if errr != nil {
		t.Fatalf("unable to query for list channels: %v", errr)
	}
	...
	pendingChanResp, errr := node.PendingChannels(ctxt, pendingChansRequest)
	if errr != nil {
		t.Fatalf("unable to query for pending channels: %v", errr)
	}
	var found bool
...
func waitForChannelPendingForceClose(ctx context.Context,
	node *lntest.HarnessNode, fundingChanPoint *rpc_pb.ChannelPoint) er.R {
	...
	return wait.NoError(func() er.R {
		...
		if err != nil {
			return er.Errorf("unable to get pending channels: %v",
				err)
		}

		forceClose, errr := findForceClosedChannel(pendingChanResp, &op)
		if errr != nil {
			return errr
		}
		...
		if forceClose.MaturityHeight == 0 {
			return er.Errorf("channel had maturity height of 0")
		}
...
type lnrpcForceCloseChannel = rpc_pb.PendingChannelsResponse_ForceClosedChannel
...
func waitForNumChannelPendingForceClose(ctx context.Context,
	...
	perChanCheck func(channel *lnrpcForceCloseChannel) er.R) er.R {

	return wait.NoError(func() er.R {
		resp, err := node.PendingChannels(
			ctx, &rpc_pb.PendingChannelsRequest{},
		)
		if err != nil {
			return er.Errorf("unable to get pending channels: %v",
				err)
		...
		if len(forceCloseChans) != expectedNum {
			return er.Errorf("bob should have %d pending "+
...
func cleanupForceClose(t *harnessTest, net *lntest.NetworkHarness,
	node *lntest.HarnessNode, chanPoint *rpc_pb.ChannelPoint) {
...
func numOpenChannelsPending(ctxt context.Context, node *lntest.HarnessNode) (int, er.R) {
	pendingChansRequest := &rpc_pb.PendingChannelsRequest{}
...
func assertNumOpenChannelsPending(ctxt context.Context, t *harnessTest,
	...
	err := wait.NoError(func() er.R {
		...
		if !bobStateCorrect {
			return er.Errorf("number of pending channels for bob "+
...
func assertNumConnections(t *harnessTest, alice, bob *lntest.HarnessNode,
	...
	for i := nPolls - 1; i >= 0; i-- {
		...
		case <-tick.C:
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			aNumPeers, err := alice.ListPeers(ctxt, &rpc_pb.ListPeersRequest{})
			if err != nil {
			...
			ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
			bNumPeers, err := bob.ListPeers(ctxt, &rpc_pb.ListPeersRequest{})
			if err != nil {
...
func completePaymentRequests(ctx context.Context, client rpc_pb.LightningClient,
	routerClient routerrpc_pb.RouterClient, paymentRequests []string,
	awaitResponse bool) er.R {

	// We start by getting the current state of the client's channels. This
	// is needed to ensure the payments actually have been committed before
	// we return.
	ctxt, _ := context.WithTimeout(ctx, defaultTimeout)
	req := &rpc_pb.ListChannelsRequest{}
	listResp, errr := client.ListChannels(ctxt, req)
	if errr != nil {
		return er.E(errr)
	}

	// send sends a payment and returns an error if it doesn't succeeded.
	send := func(payReq string) er.R {
		ctxc, cancel := context.WithCancel(ctx)
		defer cancel()

		payStream, errr := routerClient.SendPaymentV2(
			ctxc,
		...
		if resp.Status != rpc_pb.Payment_SUCCEEDED {
			return er.Errorf("%v", resp.FailureReason)
		}
	...
	err := wait.Predicate(func() bool {
		ctxt, _ = context.WithTimeout(ctx, defaultTimeout)
		newListResp, errr := client.ListChannels(ctxt, req)
		if errr != nil {
			return false
		}
...
func createPayReqs(node *lntest.HarnessNode, paymentAmt btcutil.Amount,
	numInvoices int) ([]string, [][]byte, []*rpc_pb.Invoice, er.R) {

	payReqs := make([]string, numInvoices)
	rHashes := make([][]byte, numInvoices)
	invoices := make([]*rpc_pb.Invoice, numInvoices)
	for i := 0; i < numInvoices; i++ {
		preimage := make([]byte, 32)
		_, err := rand.Read(preimage)
		if err != nil {
			return nil, nil, nil, er.Errorf("unable to generate "+
				"preimage: %v", err)
		}
		...
		if err != nil {
			return nil, nil, nil, er.Errorf("unable to add "+
				"invoice: %v", err)
		}
...
func getChanInfo(ctx context.Context, node *lntest.HarnessNode) (
	*rpc_pb.Channel, er.R) {
	...
	req := &rpc_pb.ListChannelsRequest{}
	channelInfo, errr := node.ListChannels(ctx, req)
	if errr != nil {
		return nil, er.E(errr)
	}
	if len(channelInfo.Channels) != 1 {
		return nil, er.Errorf("node should only have a single "+
...
func testGetRecoveryInfo(net *lntest.NetworkHarness, t *harnessTest) {
	...
	checkInfo := func(expectedRecoveryMode, expectedRecoveryFinished bool,
		...
		err = wait.Predicate(func() bool {
			...
			req := &rpc_pb.GetRecoveryInfoRequest{}
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	restoreCheckBalance := func(expAmount int64, expectedNumUTXOs uint32,
		...
		err = wait.Predicate(func() bool {
			req := &rpc_pb.WalletBalanceRequest{}
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			...
			utxoReq := &rpc_pb.ListUnspentRequest{
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			newP2WKHAddrReq := &rpc_pb.NewAddressRequest{
				Type: AddrTypeWitnessPubkeyHash,
			}

			newNP2WKHAddrReq := &rpc_pb.NewAddressRequest{
			...
			for i := 0; i < nskip; i++ {
				ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
				_, errr := node.NewAddress(ctxt, newP2WKHAddrReq)
				if errr != nil {
					t.Fatalf("unable to generate new "+
						"p2wkh address: %v", errr)
				}

				ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
				_, errr = node.NewAddress(ctxt, newNP2WKHAddrReq)
				if errr != nil {
					t.Fatalf("unable to generate new "+
						"np2wkh address: %v", errr)
				}
			}
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(
				ctxt, btcutil.UnitsPerCoin(), node,
			)
			if err != nil {
	...
	promptChangeAddr := func(node *lntest.HarnessNode) {
		...
		ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
		resp, errr := node.SendCoins(ctxt, &rpc_pb.SendCoinsRequest{
			Addr:   minerAddr.String(),
			Amount: minerAmt,
		})
		if errr != nil {
			t.Fatalf("unable to send coins to miner: %v", errr)
		}
...
func channelCommitType(node *lntest.HarnessNode,
	chanPoint *rpc_pb.ChannelPoint) (commitType, er.R) {
	...
	if err != nil {
		return 0, er.Errorf("listchannels failed: %v", err)
	}
	...
	for _, c := range channels.Channels {
		if c.ChannelPoint == txStr(chanPoint) {
			...
			case rpc_pb.CommitmentType_ANCHORS:
				return commitTypeAnchors, nil

			// StaticRemoteKey means it is tweakless,
			case rpc_pb.CommitmentType_STATIC_REMOTE_KEY:
	...
	return 0, er.Errorf("channel point %v not found", chanPoint)
}

// assertChannelBalanceResp makes a ChannelBalance request and checks the
// returned response matches the expected.
func assertChannelBalanceResp(t *harnessTest,
	node *lntest.HarnessNode, expected *rpc_pb.ChannelBalanceResponse) {
...
func getChannelBalance(t *harnessTest,
	node *lntest.HarnessNode) *rpc_pb.ChannelBalanceResponse {

	t.t.Helper()

	ctxt, _ := context.WithTimeout(context.Background(), defaultTimeout)
	req := &rpc_pb.ChannelBalanceRequest{}
...
func basicChannelFundingTest(t *harnessTest, net *lntest.NetworkHarness,
	...
	fundingShim *rpc_pb.FundingShim) (*rpc_pb.Channel, *rpc_pb.Channel, func(), er.R) {
	...
	checkChannelBalance := func(node *lntest.HarnessNode,
		oldChannelBalance *rpc_pb.ChannelBalanceResponse,
	...
	if err != nil {
		return nil, nil, nil, er.Errorf("alice didn't report "+
			"channel: %v", err)
	}
	err = bob.WaitForNetworkChannelOpen(ctxt, chanPoint)
	if err != nil {
		return nil, nil, nil, er.Errorf("bob didn't report "+
			"channel: %v", err)
	}

	cType, err := channelCommitType(alice, chanPoint)
	if err != nil {
		return nil, nil, nil, er.Errorf("unable to get channel "+
			"type: %v", err)
	}
	...
	req := &rpc_pb.ListChannelsRequest{}
	aliceChannel, errr := alice.ListChannels(context.Background(), req)
	if errr != nil {
		return nil, nil, nil, er.Errorf("unable to obtain chan: %v", errr)
	}

	bobChannel, errr := bob.ListChannels(context.Background(), req)
	if errr != nil {
		return nil, nil, nil, er.Errorf("unable to obtain chan: %v", errr)
	}
		...
		for _, daveCommitType := range allTypes {
			...
			success := t.t.Run(testName, func(t *testing.T) {
				...
				case expType == commitTypeAnchors &&
					chansCommitType == rpc_pb.CommitmentType_ANCHORS:

				case expType == commitTypeTweakless &&
					chansCommitType == rpc_pb.CommitmentType_STATIC_REMOTE_KEY:

				case expType == commitTypeLegacy &&
					chansCommitType == rpc_pb.CommitmentType_LEGACY:
...
func testUnconfirmedChannelFunding(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// change output.
	addrReq := &rpc_pb.NewAddressRequest{
		Type: rpc_pb.AddressType_WITNESS_PUBKEY_HASH,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr := carol.NewAddress(ctxt, addrReq)
	if errr != nil {
		t.Fatalf("unable to get new address: %v", errr)
	}

	sendReq := &rpc_pb.SendCoinsRequest{
		Addr:   resp.Address,
		Amount: int64(chanAmt) / 5,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	_, errr = carol.SendCoins(ctxt, sendReq)
	if errr != nil {
		t.Fatalf("unable to send coins: %v", errr)
	}
	...
	checkChannelBalance := func(node *lntest.HarnessNode,
		...
		expectedResponse := &rpc_pb.ChannelBalanceResponse{
...
func testPaymentFollowingChannelOpen(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// executed.
	sendAndAssertSuccess(t, net.Alice, &routerrpc_pb.SendPaymentRequest{
	...
	ctxt, cancel = context.WithTimeout(ctxb, defaultTimeout)
	defer cancel()
	res, errr := net.Bob.ListChannels(ctxt, &rpc_pb.ListChannelsRequest{})
	if errr != nil {
		t.Fatalf("unable to list bob channels: %v", errr)
	}
...
func txStr(chanPoint *rpc_pb.ChannelPoint) string {
...
type expectedChanUpdate struct {
	...
	expectedPolicy  *rpc_pb.RoutingPolicy
	chanPoint       *rpc_pb.ChannelPoint
}
...
func getChannelPolicies(t *harnessTest, node *lntest.HarnessNode,
	advertisingNode string,
	chanPoints ...*rpc_pb.ChannelPoint) []*rpc_pb.RoutingPolicy {

	ctxb := context.Background()

	descReq := &rpc_pb.ChannelGraphRequest{
	...
	var policies []*rpc_pb.RoutingPolicy
out:
...
func assertChannelPolicy(t *harnessTest, node *lntest.HarnessNode,
	advertisingNode string, expectedPolicy *rpc_pb.RoutingPolicy,
	chanPoints ...*rpc_pb.ChannelPoint) {
...
func testUpdateChannelPolicy(net *lntest.NetworkHarness, t *harnessTest) {
	...
	expectedPolicyBob := &rpc_pb.RoutingPolicy{
		FeeBaseMsat:      defaultFeeBase,
		FeeRateMilliMsat: defaultFeeRate,
		TimeLockDelta:    defaultTimeLockDelta,
		MinHtlc:          customMinHtlc,
	...
	expectedPolicyCarol := &rpc_pb.RoutingPolicy{
		FeeBaseMsat:      defaultFeeBase,
		FeeRateMilliMsat: defaultFeeRate,
		TimeLockDelta:    defaultTimeLockDelta,
		MinHtlc:          defaultMinHtlc,
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr := carol.AddInvoice(ctxt, invoice)
	if errr != nil {
	...
	expErr := rpc_pb.PaymentFailureReason_FAILURE_REASON_NO_ROUTE
	if strings.Contains(err.String(), expErr.String()) {
	...
	routesReq := &rpc_pb.QueryRoutesRequest{
		PubKey:         carol.PubKey[:],
		Amt:            int64(payAmt),
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	routes, errr := net.Alice.QueryRoutes(ctxt, routesReq)
	if errr != nil {
		t.Fatalf("unable to get route: %v", errr)
	}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	alicePayStream, errr := net.Alice.SendToRoute(ctxt)
	if errr != nil {
		t.Fatalf("unable to create payment stream for alice: %v", errr)
	}
	sendReq := &rpc_pb.SendToRouteRequest{
		PaymentHash: resp.RHash,
		Route:       routes.Routes[0],
	}

	errr = alicePayStream.Send(sendReq)
	if errr != nil {
		t.Fatalf("unable to send payment: %v", errr)
	}
	...
	sendResp, errr := alicePayStream.Recv()
	if errr != nil {
		t.Fatalf("unable to send payment: %v", errr)
	}
	...
	req := &rpc_pb.PolicyUpdateRequest{
		BaseFeeMsat:   baseFee,
		FeeRate:       float64(feeRate),
		TimeLockDelta: timeLockDelta,
		MaxHtlcMsat:   maxHtlc,
		Scope: &rpc_pb.PolicyUpdateRequest_ChanPoint{
			ChanPoint: chanPoint,
		},
	}
	...
	req.Scope = &rpc_pb.PolicyUpdateRequest_Global{}

	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	_, errr = net.Alice.UpdateChannelPolicy(ctxt, req)
	if errr != nil {
		t.Fatalf("unable to update alice's channel policy: %v", errr)
	}
...
func waitForNodeBlockHeight(ctx context.Context, node *lntest.HarnessNode,
	...
	err := wait.Predicate(func() bool {
		...
		info, err := node.GetInfo(ctxt, &rpc_pb.GetInfoRequest{})
		if err != nil {
			predErr = er.E(err)
			return false
		}
		...
		if int32(info.BlockHeight) != height {
			predErr = er.Errorf("expected block height to "+
...
func assertMinerBlockHeightDelta(t *harnessTest,
	...
	err := wait.Predicate(func() bool {
		...
		if err != nil {
			predErr = er.Errorf("unable to get current "+
				"blockheight %v", err)
			return false
		}
		...
		if tempMinerHeight != minerHeight+delta {
			predErr = er.Errorf("expected new miner(%d) to be %d "+
...
func testOpenChannelAfterReorg(net *lntest.NetworkHarness, t *harnessTest) {
	...
	util.RequireNoErr(t.t, err, "failed to create temp miner")
	defer func() {
		util.RequireNoErr(
	...
	req := &rpc_pb.ChannelGraphRequest{
		IncludeUnannounced: true,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	chanGraph, errr := net.Alice.DescribeGraph(ctxt, req)
	if errr != nil {
		t.Fatalf("unable to query for alice's routing table: %v", errr)
	}
	...
	err = wait.Predicate(func() bool {
		...
		chanGraph, errr = net.Alice.DescribeGraph(ctxt, req)
		if errr != nil {
			predErr = er.Errorf("unable to query for alice's routing table: %v", errr)
			return false
		}

		numEdges = len(chanGraph.Edges)
		if numEdges != 0 {
			predErr = er.Errorf("expected to find no edge in the graph, found %d",
				numEdges)
			return false
		}
		return true
...
func testChannelUnsettledBalance(net *lntest.NetworkHarness, t *harnessTest) {
	...
	util.RequireNoErr(t.t, err, "unable to get channel type")
	...
	for i := 0; i < numInvoices; i++ {
		go func() {
			ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
			_, errr := net.Alice.RouterClient.SendPaymentV2(ctxt,
				&routerrpc_pb.SendPaymentRequest{
					Dest:           carolPubKey,
					Amt:            int64(payAmt),
	...
	err = wait.Predicate(func() bool {
		...
		for _, node := range nodes {
			...
			if int(chanInfo.UnsettledBalance) != expectedBalance {
				unsettledErr = er.Errorf("unsettled balance failed "+
...
func findForceClosedChannel(pendingChanResp *rpc_pb.PendingChannelsResponse,
	op *wire.OutPoint) (*rpc_pb.PendingChannelsResponse_ForceClosedChannel, er.R) {
	...
	return nil, er.New("channel not marked as force closed")
}
...
func findWaitingCloseChannel(pendingChanResp *rpc_pb.PendingChannelsResponse,
	op *wire.OutPoint) (*rpc_pb.PendingChannelsResponse_WaitingCloseChannel, er.R) {
...
func checkPendingChannelNumHtlcs(
	forceClose *rpc_pb.PendingChannelsResponse_ForceClosedChannel,
	expectedNumHtlcs int) er.R {

	if len(forceClose.PendingHtlcs) != expectedNumHtlcs {
		return er.Errorf("expected force closed channel to have %d "+
...
func checkNumForceClosedChannels(pendingChanResp *rpc_pb.PendingChannelsResponse,
	expectedNumChans int) er.R {

	if len(pendingChanResp.PendingForceClosingChannels) != expectedNumChans {
		return er.Errorf("expected to find %d force closed channels, "+
...
func checkNumWaitingCloseChannels(pendingChanResp *rpc_pb.PendingChannelsResponse,
	expectedNumChans int) er.R {

	if len(pendingChanResp.WaitingCloseChannels) != expectedNumChans {
		return er.Errorf("expected to find %d channels waiting "+
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	for _, channelType := range commitTypes {
		...
		success := t.t.Run(testName, func(t *testing.T) {
			...
			err = net.SendCoins(
				ctxt, btcutil.UnitsPerCoin(), alice,
			)
			if err != nil {
			...
			// anchor.
			err = net.SendCoins(
				ctxt, btcutil.UnitsPerCoin(), carol,
			)
			if err != nil {
				t.Fatalf("unable to send coins to Alice: %v",
					err)
...
func channelForceClosureTest(net *lntest.NetworkHarness, t *harnessTest,
	...
	carolBalReq := &rpc_pb.WalletBalanceRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalResp, errr := carol.WalletBalance(ctxt, carolBalReq)
	if errr != nil {
	...
	pendingChansRequest := &rpc_pb.PendingChannelsRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	pendingChanResp, errr := alice.PendingChannels(ctxt, pendingChansRequest)
	if err != nil {
	...
	var (
		aliceReports = make(map[string]*rpc_pb.Resolution)
		carolReports = make(map[string]*rpc_pb.Resolution)
	)
	...
	// reports.
	if channelType == commitTypeAnchors {
		aliceReports[aliceAnchor.OutPoint.String()] = &rpc_pb.Resolution{
			ResolutionType: rpc_pb.ResolutionType_ANCHOR,
			Outcome:        rpc_pb.ResolutionOutcome_CLAIMED,
	...
	err = wait.NoError(func() er.R {
		ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
		pendingChanResp, errr := alice.PendingChannels(
			ctxt, pendingChansRequest,
		)
		if errr != nil {
			return er.Errorf("unable to query for pending "+
				"channels: %v", errr)
		}
		...
		if forceClose.LimboBalance == 0 {
			return er.New("all funds should still be in limbo")
		}
		...
		if forceClose.RecoveredBalance != expectedRecoveredBalance {
			return er.New("no funds should yet be shown as recovered")
		}
	...
	if channelType == commitTypeAnchors {
		carolReports[carolAnchor.OutPoint.String()] = &rpc_pb.Resolution{
			ResolutionType: rpc_pb.ResolutionType_ANCHOR,
			Outcome:        rpc_pb.ResolutionOutcome_CLAIMED,
	...
	err = wait.NoError(func() er.R {
		...
		if forceClose.LimboBalance == 0 {
			return er.New("all funds should still be in " +
				"limbo")
		}
		expectedRecoveredBalance := int64(0)
		if channelType == commitTypeAnchors {
		...
		if forceClose.RecoveredBalance != expectedRecoveredBalance {
			return er.New("no funds should yet be shown " +
				"as recovered")
		}
	...
	aliceReports[output.String()] = &rpc_pb.Resolution{
		ResolutionType: rpc_pb.ResolutionType_COMMIT,
		Outcome:        rpc_pb.ResolutionOutcome_CLAIMED,
	...
	carolReports[carolCommit.OutPoint.String()] = &rpc_pb.Resolution{
		ResolutionType: rpc_pb.ResolutionType_COMMIT,
		Outcome:        rpc_pb.ResolutionOutcome_CLAIMED,
		Outpoint: &rpc_pb.OutPoint{
	...
	err = wait.Predicate(func() bool {
		...
		if errr != nil {
			predErr = er.Errorf("unable to query for pending "+
				"channels: %v", errr)
			return false
		}
		...
		if forceClose.LimboBalance == 0 {
			predErr = er.Errorf("expected funds in limbo, found 0")
			return false
		}
	...
	err = wait.NoError(func() er.R {
		...
		// non-zero as well.
		if forceClose.LimboBalance == 0 {
			return er.New("htlc funds should still be in " +
				"limbo")
		}
	...
	for _, htlcTxID := range htlcTxIDs {
		...
		resolutionOutpoint := &rpc_pb.OutPoint{
		...
		aliceReports[outpoint.String()] = &rpc_pb.Resolution{
			ResolutionType: rpc_pb.ResolutionType_OUTGOING_HTLC,
			Outcome:        rpc_pb.ResolutionOutcome_FIRST_STAGE,
		...
		// it has timed out.
		carolReports[outpoint.String()] = &rpc_pb.Resolution{
			ResolutionType: rpc_pb.ResolutionType_INCOMING_HTLC,
			Outcome:        rpc_pb.ResolutionOutcome_TIMEOUT,
			SweepTxid:      "",
			Outpoint:       resolutionOutpoint,
			AmountSat:      uint64(paymentAmt),
		}
	...
	err = wait.Predicate(func() bool {
		...
		if forceClose.LimboBalance == 0 {
			predErr = er.Errorf("htlc funds should still be in limbo")
			return false
		}
	...
	err = wait.Predicate(func() bool {
		...
		if pendingChanResp.TotalLimboBalance != 0 {
			predErr = er.New("no user funds should be left " +
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalResp, errr = net.Bob.WalletBalance(ctxt, carolBalReq)
	if errr != nil {
		t.Fatalf("unable to get carol's balance: %v", errr)
	}
...
func assertReports(ctxb context.Context, t *harnessTest,
	...
	expected map[string]*rpc_pb.Resolution) {
	...
	closed, err := node.ClosedChannels(
		ctxt, &rpc_pb.ClosedChannelsRequest{},
	)
	require.NoError(t.t, err)

	var resolutions []*rpc_pb.Resolution
...
func assertSweepFound(ctx context.Context, t *testing.T, node *lntest.HarnessNode,
	...
	sweepResp, err := node.WalletKitClient.ListSweeps(
		ctx, &walletrpc_pb.ListSweepsRequest{
			Verbose: verbose,
		},
	)
...
func findSweepInTxids(t *testing.T, sweepTxid string,
	sweepResp *walletrpc_pb.ListSweepsResponse) bool {
...
func assertAmountSent(amt btcutil.Amount, sndr, rcvr *lntest.HarnessNode) func() er.R {
	return func() er.R {
		...
		if rcvrSatoshisReceived != int64(amt) {
			return er.Errorf("%s's satoshis received is "+
...
func assertLastHTLCError(t *harnessTest, node *lntest.HarnessNode,
	code rpc_pb.Failure_FailureCode) {
...
func testSphinxReplayPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), fred)
	if err != nil {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	invoiceResp, errr := dave.AddInvoice(ctxt, invoice)
	if errr != nil {
		t.Fatalf("unable to add invoice: %v", errr)
	}
	...
	payStream, errr := fred.RouterClient.SendPaymentV2(
		ctx,
		&routerrpc_pb.SendPaymentRequest{
	...
	if errr != nil {
		t.Fatalf("unable to open payment stream: %v", errr)
	}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	dbInvoice, errr := dave.LookupInvoice(ctxt, payHash)
	if errr != nil {
	...
	if result.Status == rpc_pb.Payment_SUCCEEDED {
		t.Fatalf("expected payment error")
	}
	assertLastHTLCError(t, fred, rpc_pb.Failure_INVALID_ONION_KEY)
...
func assertChannelConstraintsEqual(
	t *harnessTest, want, got *rpc_pb.ChannelConstraints) {
...
func testListChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxb = context.Background()
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr := alice.ListChannels(ctxt, listReq)
	if errr != nil {
	...
	defaultConstraints := &rpc_pb.ChannelConstraints{
		CsvDelay:          4,
		ChanReserveSat:    1000,
	...
	customizedConstraints := &rpc_pb.ChannelConstraints{
		CsvDelay:          4,
		ChanReserveSat:    1000,
		DustLimitSat:      uint64(lnwallet.DefaultDustLimit()),
		MaxPendingAmtMsat: 99000000,
	...
	ctxb = context.Background()
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr = bob.ListChannels(ctxt, listReq)
	if errr != nil {
...
func testListPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	delPaymentsReq := &rpc_pb.DeleteAllPaymentsRequest{}
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	...
	reqInit := &rpc_pb.ListPaymentsRequest{}
	...
	// above.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	if errr := net.Alice.WaitForNetworkChannelOpen(ctxt, chanPoint); errr != nil {
		t.Fatalf("alice didn't advertise channel before "+
			"timeout: %v", errr)
	}
	if errr := net.Bob.WaitForNetworkChannelOpen(ctxt, chanPoint); errr != nil {
		t.Fatalf("bob didn't advertise channel before "+
			"timeout: %v", errr)
	}
	...
	delReq := &rpc_pb.DeleteAllPaymentsRequest{}
	ctxt, _ = context.WithTimeout(ctxt, defaultTimeout)
	...
	listReq := &rpc_pb.ListPaymentsRequest{}
	ctxt, _ = context.WithTimeout(ctxt, defaultTimeout)
...
func assertAmountPaid(t *harnessTest, channelName string,
	...
	checkAmountPaid := func() er.R {
		listReq := &rpc_pb.ListChannelsRequest{}
		ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
		resp, err := node.ListChannels(ctxt, listReq)
		if err != nil {
			return er.Errorf("unable to for node's "+
				"channels: %v", err)
		}
		for _, channel := range resp.Channels {
			...
			if channel.TotalSatoshisSent != amountSent {
				return er.Errorf("%v: incorrect amount"+
		...
		return er.Errorf("channel not found")
	}
...
func updateChannelPolicy(t *harnessTest, node *lntest.HarnessNode,
	chanPoint *rpc_pb.ChannelPoint, baseFee int64, feeRate int64,
	...
	updateFeeReq := &rpc_pb.PolicyUpdateRequest{
		BaseFeeMsat:   baseFee,
...
func testSingleHopSendToRouteCase(net *lntest.NetworkHarness, t *harnessTest,
	...
	for _, payReq := range payReqs {
		...
		resp, err := dave.DecodePayReq(
			ctx,
			&rpc_pb.PayReqString{PayReq: payReq},
		)
		if err != nil {
	...
	ctxt, cancel := context.WithTimeout(ctxb, defaultTimeout)
	defer cancel()
	util.RequireNoErr(t.t, waitForNodeBlockHeight(ctxt, carol, minerHeight))
	util.RequireNoErr(t.t, waitForNodeBlockHeight(ctxt, dave, minerHeight))
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	routes, errr := carol.QueryRoutes(ctxt, routesReq)
	if errr != nil {
	...
	// hash and amount.
	ctxt, _ = context.WithTimeout(ctxt, defaultTimeout)
	paymentsResp, errr := carol.ListPayments(
		ctxt, &rpc_pb.ListPaymentsRequest{},
	)
	if errr != nil {
	...
	// hash and amount.
	ctxt, _ = context.WithTimeout(ctxt, defaultTimeout)
	invoicesResp, errr := dave.ListInvoices(
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), net.Bob)
	if err != nil {
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), charlie)
	if err != nil {
	...
	// for Alice -> Bob.
	fakeReq := &rpc_pb.QueryRoutesRequest{
		PubKey: charlie.PubKey[:],
		Amt:    int64(1),
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	fakeRoute, errr := carol.QueryRoutes(ctxt, fakeReq)
	if errr != nil {
		t.Fatalf("unable get fake route: %v", errr)
	}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr := net.Bob.AddInvoice(ctxt, invoice)
	if errr != nil {
		t.Fatalf("unable to add invoice: %v", errr)
	}
...
func testUnannouncedChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	chanGraph, errr := net.Alice.DescribeGraph(ctxt, req)
	if errr != nil {
		t.Fatalf("unable to query alice's graph: %v", errr)
	}
	...
	err = wait.Predicate(func() bool {
		...
		chanGraph, errr = net.Alice.DescribeGraph(ctxt, req)
		if errr != nil {
			predErr = er.Errorf("unable to query alice's graph: %v", errr)
			return false
		}

		numEdges = len(chanGraph.Edges)
		if numEdges != 1 {
			predErr = er.Errorf("expected to find 1 announced edge in "+
...
func testPrivateChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), dave)
	if err != nil {
	...
	err = wait.Predicate(func() bool {
		...
		if aliceChans != 4 {
			predErr = er.Errorf("expected Alice to know 4 edges, "+
				"had %v", aliceChans)
			return false
		}
		alicePubChans := numChannels(net.Alice, false)
		if alicePubChans != 3 {
			predErr = er.Errorf("expected Alice to know 3 public edges, "+
				"had %v", alicePubChans)
			return false
		}
		bobChans := numChannels(net.Bob, true)
		if bobChans != 3 {
			predErr = er.Errorf("expected Bob to know 3 edges, "+
				"had %v", bobChans)
			return false
		}
		carolChans := numChannels(carol, true)
		if carolChans != 4 {
			predErr = er.Errorf("expected Carol to know 4 edges, "+
				"had %v", carolChans)
			return false
		}
		carolPubChans := numChannels(carol, false)
		if carolPubChans != 3 {
			predErr = er.Errorf("expected Carol to know 3 public edges, "+
				"had %v", carolPubChans)
			return false
		}
		daveChans := numChannels(dave, true)
		if daveChans != 3 {
			predErr = er.Errorf("expected Dave to know 3 edges, "+
				"had %v", daveChans)
			return false
		}
		return true
...
func testInvoiceRoutingHints(net *lntest.NetworkHarness, t *harnessTest) {
	...
	aliceChans := []*rpc_pb.ChannelPoint{
	...
	listReq := &rpc_pb.ListChannelsRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	listResp, errr := net.Alice.ListChannels(ctxt, listReq)
	if errr != nil {
		t.Fatalf("unable to retrieve alice's channels: %v", errr)
	}
...
func testMultiHopOverPrivateChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	if errr != nil {
		t.Fatalf("unable to add invoice for dave: %v", errr)
	}
...
func testInvoiceSubscriptions(net *lntest.NetworkHarness, t *harnessTest) {
	...
	req := &rpc_pb.InvoiceSubscription{}
	ctx, cancelInvoiceSubscription := context.WithCancel(ctxb)
	bobInvoiceSubscription, errr := net.Bob.SubscribeInvoices(ctx, req)
	if errr != nil {
		t.Fatalf("unable to subscribe to bob's invoice updates: %v", errr)
	}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	stream, errr := net.Alice.RouterClient.SendPaymentV2(ctxt, sendReq)
	if errr != nil {
		close(quit)
		t.Fatalf("unable to send payment: %v", errr)
	}
...
type channelSubscription struct {
	updateChan chan *rpc_pb.ChannelEventUpdate
	errChan    chan er.R
	quit       chan struct{}
}
...
func subscribeChannelNotifications(ctxb context.Context, t *harnessTest,
	...
	// closed.
	req := &rpc_pb.ChannelEventSubscription{}
	...
	quit := make(chan struct{})
	chanUpdates := make(chan *rpc_pb.ChannelEventUpdate, 20)
	go func() {
		defer cancelFunc()
		for {
			select {
...
// expected type.
func verifyCloseUpdate(chanUpdate *rpc_pb.ChannelEventUpdate,
	closeType rpc_pb.ChannelCloseSummary_ClosureType,
	closeInitiator rpc_pb.Initiator) er.R {

	// We should receive one inactive and one closed notification
	// for each channel.
	switch update := chanUpdate.Channel.(type) {
	case *rpc_pb.ChannelEventUpdate_InactiveChannel:
		if chanUpdate.Type != rpc_pb.ChannelEventUpdate_INACTIVE_CHANNEL {
			return er.Errorf("update type mismatch: expected %v, got %v",
				rpc_pb.ChannelEventUpdate_INACTIVE_CHANNEL,
				chanUpdate.Type)
		}
	case *rpc_pb.ChannelEventUpdate_ClosedChannel:
		if chanUpdate.Type !=
			rpc_pb.ChannelEventUpdate_CLOSED_CHANNEL {
			return er.Errorf("update type mismatch: expected %v, got %v",
				rpc_pb.ChannelEventUpdate_CLOSED_CHANNEL,
				chanUpdate.Type)
		}
		...
		if update.ClosedChannel.CloseType != closeType {
			return er.Errorf("channel closure type "+
		...
		if update.ClosedChannel.CloseInitiator != closeInitiator {
			return er.Errorf("expected close intiator: %v, got: %v",
				closeInitiator,
				update.ClosedChannel.CloseInitiator)
		}

	default:
		return er.Errorf("channel update channel of wrong type, "+
...
func testBasicChannelCreationAndUpdates(net *lntest.NetworkHarness, t *harnessTest) {
	...
	chanPoints := make([]*rpc_pb.ChannelPoint, numChannels)
	...
	verifyOpenUpdatesReceived := func(sub channelSubscription) er.R {
		numChannelUpds = 0
		for numChannelUpds < totalNtfns {
			select {
			case update := <-sub.updateChan:
				switch update.Type {
				case rpc_pb.ChannelEventUpdate_PENDING_OPEN_CHANNEL:
				...
				case rpc_pb.ChannelEventUpdate_OPEN_CHANNEL:
				...
				case rpc_pb.ChannelEventUpdate_ACTIVE_CHANNEL:
				...
				default:
					return er.Errorf("update type mismatch: "+
				...
				numChannelUpds++
			case <-time.After(time.Second * 10):
				return er.Errorf("timeout waiting for channel "+
	...
	verifyCloseUpdatesReceived := func(sub channelSubscription,
		forceType rpc_pb.ChannelCloseSummary_ClosureType,
		closeInitiator rpc_pb.Initiator) er.R {
		...
		for numChannelUpds < 2*numChannels {
			expectedCloseType := rpc_pb.ChannelCloseSummary_COOPERATIVE_CLOSE
			...
			case err := <-sub.errChan:
				return err
			case <-time.After(time.Second * 10):
				return er.Errorf("timeout waiting "+
	...
	if err := verifyCloseUpdatesReceived(bobChanSub,
		rpc_pb.ChannelCloseSummary_REMOTE_FORCE_CLOSE,
		rpc_pb.Initiator_INITIATOR_REMOTE); err != nil {
	...
	if err := verifyCloseUpdatesReceived(aliceChanSub,
		rpc_pb.ChannelCloseSummary_LOCAL_FORCE_CLOSE,
		rpc_pb.Initiator_INITIATOR_LOCAL); err != nil {
		t.Fatalf("errored verifying close updates: %v", err)
	}
}
...
func testMaxPendingChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	openStreams := make([]rpc_pb.Lightning_OpenChannelClient, maxPendingChannels)
	...
	if err == nil {
		...
		err.String(), lnwire.ErrMaxPendingChannels.Detail,
	) {
	...
	chanPoints := make([]*rpc_pb.ChannelPoint, maxPendingChannels)
...
func getNTxsFromMempool(miner *rpcclient.Client, n int,
	timeout time.Duration) ([]*wire.MsgTx, er.R) {
...
func getTxFee(miner *rpcclient.Client, tx *wire.MsgTx) (btcutil.Amount, er.R) {
...
func testFailingChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	err = wait.Predicate(func() bool {
		pendingChansRequest := &rpc_pb.PendingChannelsRequest{}
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
		pendingChanResp, errr := net.Alice.PendingChannels(ctxt,
			pendingChansRequest)
		if errr != nil {
			predErr = er.Errorf("unable to query for pending "+
				"channels: %v", errr)
		...
		if n != 1 {
			predErr = er.Errorf("Expected to find %d channels "+
	...
	err = wait.Predicate(func() bool {
		...
		if n != 1 {
			predErr = er.Errorf("expected to find %d channel "+
...
func testGarbageCollectLinkNodes(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// channel graph.
	describeGraphReq := &rpc_pb.ChannelGraphRequest{
		IncludeUnannounced: true,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	channelGraph, errr := net.Alice.DescribeGraph(ctxt, describeGraphReq)
	if errr != nil {
...
func testRevokedCloseRetribution(net *lntest.NetworkHarness, t *harnessTest) {
	...
	err = wait.Predicate(func() bool {
		...
		if bChan.LocalBalance != 30000 {
			predErr = er.Errorf("bob's balance is incorrect, "+
	...
	bobTempDbPath, errr := ioutil.TempDir("", "bob-past-state")
	if errr != nil {
		t.Fatalf("unable to create temp db folder: %v", errr)
	}
	...
	if err = net.RestartNode(net.Bob, func() er.R {
	...
	var closeUpdates rpc_pb.Lightning_CloseChannelClient
	force := true
	err = wait.Predicate(func() bool {
		ctxt, _ := context.WithTimeout(ctxb, channelCloseTimeout)
...
func testRevokedCloseRetributionZeroValueRemoteOutput(net *lntest.NetworkHarness,
	...
	// particular point in history.
	carolTempDbPath, errr := ioutil.TempDir("", "carol-past-state")
	if errr != nil {
		t.Fatalf("unable to create temp db folder: %v", errr)
	}
	...
	// within the channel's history.
	if err = net.RestartNode(carol, func() er.R {
	...
	var (
		closeUpdates rpc_pb.Lightning_CloseChannelClient
...
func testRevokedCloseRetributionRemoteHodl(net *lntest.NetworkHarness,
	...
	errNotFound := er.GenericErrorType.Code("justice tx not found")
	findJusticeTx := func() (*chainhash.Hash, er.R) {
		mempool, err := net.Miner.Node.GetRawMempool()
		if err != nil {
			return nil, er.Errorf("unable to get mempool from "+
				"miner: %v", err)
		}
		...
		for _, txid := range mempool {
			...
			if err != nil {
				return nil, er.Errorf("unable to query for "+
					"txs: %v", err)
			}
		...
		return nil, errNotFound.Default()
	}
	...
	if err != nil && errNotFound.Is(predErr) {
...
func testRevokedCloseRetributionAltruistWatchtower(net *lntest.NetworkHarness,
	...
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	willyInfo, errr := willy.Watchtower.GetInfo(
		ctxt, &watchtower_pb.GetInfoRequest{},
	)
	if errr != nil {
		t.Fatalf("unable to getinfo from willy: %v", errr)
	}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	addTowerReq := &wtclientrpc_pb.AddTowerRequest{
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	daveBalReq := &rpc_pb.WalletBalanceRequest{}
	daveBalResp, errr := dave.WalletBalance(ctxt, daveBalReq)
	if errr != nil {
		t.Fatalf("unable to get dave's balance: %v", errr)
	}
	...
	err = wait.NoError(func() er.R {
		ctxt, cancel := context.WithTimeout(ctxb, defaultTimeout)
		defer cancel()
		bkpStats, errr := dave.WatchtowerClient.Stats(ctxt, &wtclientrpc_pb.StatsRequest{})
		if errr != nil {
			return er.E(errr)

		}
		if bkpStats == nil {
			return er.New("no active backup sessions")
		}
		if bkpStats.NumBackups == 0 {
			return er.New("no backups accepted")
		}
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	willyBalReq := &rpc_pb.WalletBalanceRequest{}
	willyBalResp, errr := willy.WalletBalance(ctxt, willyBalReq)
	if errr != nil {
		t.Fatalf("unable to get willy's balance: %v", errr)
	}
	...
	err = wait.Invariant(func() bool {
		...
		if willyBalResp.ConfirmedBalance != 0 {
			predErr = er.Errorf("Expected Willy to have no funds "+
	...
	err = wait.Predicate(func() bool {
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
		daveBalReq := &rpc_pb.ChannelBalanceRequest{}
		...
		if daveBalResp.LocalBalance.Sat != 0 {
			predErr = er.Errorf("Dave should end up with zero "+
	...
	err = wait.Predicate(func() bool {
		...
		if daveBalResp.ConfirmedBalance <= davePreSweepBalance {
			predErr = er.Errorf("Dave should have more than %d "+
...
func assertDLPExecuted(net *lntest.NetworkHarness, t *harnessTest,
	...
	if err != nil && !strings.Contains(err.String(), "already connected") {
	...
	daveBalResp, errr := dave.WalletBalance(ctxt, balReq)
	if errr != nil {
		t.Fatalf("unable to get dave's balance: %v", errr)
	}
...
func testDataLossProtection(net *lntest.NetworkHarness, t *harnessTest) {
	...
	timeTravel := func(node *lntest.HarnessNode) (func() er.R,
		*rpc_pb.ChannelPoint, int64, er.R) {
		...
		err = wait.Predicate(func() bool {
			...
			if bChan.LocalBalance != 30000 {
				predErr = er.Errorf("balance is incorrect, "+
					"got %v, expected %v",
		...
		// particular point in history.
		tempDbPath, errr := ioutil.TempDir("", node.Name()+"-past-state")
		if errr != nil {
			t.Fatalf("unable to create temp db folder: %v", errr)
		}
		...
		// history.
		if err = net.RestartNode(node, func() er.R {
		...
		balReq := &rpc_pb.WalletBalanceRequest{}
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
		balResp, errr := node.WalletBalance(ctxt, balReq)
		if errr != nil {
			t.Fatalf("unable to get dave's balance: %v", errr)
		}
	...
	err = wait.NoError(func() er.R {
		...
		if err != nil {
			return er.Errorf("unable to get dave's balance: %v",
				err)
		}

		daveBalance := daveBalResp.ConfirmedBalance
		if daveBalance <= daveStartingBalance {
			return er.Errorf("expected dave to have balance "+
...
func assertNodeNumChannels(t *harnessTest, node *lntest.HarnessNode,
	...
	pred := func() bool {
		...
		if err != nil {
			predErr = er.Errorf("unable to query for node's "+
				"channels: %v", err)
			return false
		}
		...
		if num != numChannels {
			predErr = er.Errorf("expected %v channels, got %v",
				numChannels, num)
			return false
		}
		return true
...
func testRejectHTLC(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Send coins to Carol.
	err = net.SendCoins(ctxb, btcutil.UnitsPerCoin(), carol)
	if err != nil {
		t.Fatalf("unable to send coins to carol: %v", err)
	}

	// Send coins to Alice.
	err = net.SendCoins(ctxb, btcutil.UnitsPerCoin()/100, net.Alice)
	if err != nil {
		t.Fatalf("unable to send coins to alice: %v", err)
	}
	...
	genPreImage := func() []byte {
		...
		_, errr := rand.Read(preimage)
		if errr != nil {
			t.Fatalf("unable to generate preimage: %v", errr)
		}
	...
	resp, errr := carol.AddInvoice(ctxb, carolInvoice)
	if errr != nil {
		t.Fatalf("unable to add invoice: %v", errr)
	}
	...
	resp, errr = net.Bob.AddInvoice(ctxb, bobInvoice)
	if errr != nil {
		t.Fatalf("unable to add invoice: %v", errr)
	}
	...
	assertLastHTLCError(t, net.Alice, rpc_pb.Failure_CHANNEL_DISABLED)
...
type graphSubscription struct {
	updateChan chan *rpc_pb.GraphTopologyUpdate
	errChan    chan error
	quit       chan struct{}
}
...
func subscribeGraphNotifications(t *harnessTest, ctxb context.Context,
	...
	req := &rpc_pb.GraphTopologySubscription{}
	ctx, cancelFunc := context.WithCancel(ctxb)
	...
	quit := make(chan struct{})
	graphUpdates := make(chan *rpc_pb.GraphTopologyUpdate, 20)
	go func() {
		for {
			defer cancelFunc()
...
func testNodeSignVerify(net *lntest.NetworkHarness, t *harnessTest) {
	...
	sigReq := &rpc_pb.SignMessageRequest{MsgBin: aliceMsg}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	...
	// connected.
	verifyReq := &signrpc_pb.VerifyMessageReq{Msg: aliceMsg, Signature: []byte(aliceSig)}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	...
	*/
	...
	sigReq = &rpc_pb.SignMessageRequest{MsgBin: carolMsg}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	...
	verifyReq = &signrpc_pb.VerifyMessageReq{Msg: carolMsg, Signature: []byte(carolSig)}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	verifyResp, err = net.Bob.SignerClient.VerifyMessage(ctxt, verifyReq)
	if err != nil {
		t.Fatalf("VerifyMessage failed: %v", err)
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	now := time.Now()
	errChan := make(chan er.R)
	statusChan := make(chan *rpc_pb.Payment)
	for i := 0; i < numInvoices; i++ {
		payReq := bobPayReqs[i]
		go func() {
			ctxt, _ = context.WithTimeout(ctxb, lntest.AsyncBenchmarkTimeout)
			stream, errr := net.Alice.RouterClient.SendPaymentV2(
				ctxt,
				&routerrpc_pb.SendPaymentRequest{
					PaymentRequest: payReq,
					TimeoutSeconds: 60,
...
func testBidirectionalAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	if errr != nil {
		t.Fatalf("unable to reset mc for alice: %v", errr)
	}
	...
	send := func(node *lntest.HarnessNode, payReq string) {
		go func() {
			...
			stream, errr := node.RouterClient.SendPaymentV2(
				ctxt,
				&routerrpc_pb.SendPaymentRequest{
					PaymentRequest: payReq,
					TimeoutSeconds: 60,
...
func assertActiveHtlcs(nodes []*lntest.HarnessNode, payHashes ...[]byte) er.R {
	ctxb := context.Background()
	...
	for _, node := range nodes {
		...
		if err != nil {
			return er.Errorf("unable to get node chans: %v", err)
		}
		...
		for _, channel := range nodeChans.Channels {
			...
			for _, htlc := range channel.PendingHtlcs {
				...
				if ok {
					return er.Errorf("duplicate HashLock")
				}
			...
			if len(payHashes) != len(htlcHashes) {
				return er.Errorf("node %x had %v htlcs active, "+
			...
			for _, payHash := range payHashes {
				...
				return er.Errorf("node %x didn't have the "+
...
func assertNumActiveHtlcsChanPoint(node *lntest.HarnessNode,
	chanPoint wire.OutPoint, numHtlcs int) er.R {
	ctxb := context.Background()

	req := &rpc_pb.ListChannelsRequest{}
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	nodeChans, errr := node.ListChannels(ctxt, req)
	if errr != nil {
		return er.E(errr)
	}
	...
	for _, channel := range nodeChans.Channels {
		...
		if len(channel.PendingHtlcs) != numHtlcs {
			return er.Errorf("expected %v active HTLCs, got %v",
				numHtlcs, len(channel.PendingHtlcs))
		}
		return nil
	}

	return er.Errorf("channel point %v not found", chanPoint)
}

func assertNumActiveHtlcs(nodes []*lntest.HarnessNode, numHtlcs int) er.R {
	ctxb := context.Background()
	...
	for _, node := range nodes {
		...
		for _, channel := range nodeChans.Channels {
			if len(channel.PendingHtlcs) != numHtlcs {
				return er.Errorf("expected %v HTLCs, got %v",
					numHtlcs, len(channel.PendingHtlcs))
			}
		}
	}
...
func testSwitchCircuitPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	finalInvoice := &rpc_pb.Invoice{
		Memo:  "testing",
		Value: paymentAmt,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	resp, errr := carol.AddInvoice(ctxt, finalInvoice)
	if errr != nil {
		t.Fatalf("unable to add invoice: %v", errr)
	}
...
func testQueryRoutes(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	routesRes, errr := net.Alice.QueryRoutes(ctxt, routesReq)
	if errr != nil {
		t.Fatalf("unable to get route: %v", errr)
	}
...
func testRouteFeeCutoff(net *lntest.NetworkHarness, t *harnessTest) {
	...
	listReq := &rpc_pb.ListChannelsRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	listResp, errr := net.Bob.ListChannels(ctxt, listReq)
	if errr != nil {
		t.Fatalf("unable to retrieve bob's channels: %v", errr)
	}
	...
	checkRoute := func(route *rpc_pb.Route) {
		if len(route.Hops) != 2 {
	...
	// payments.
	testFeeCutoff := func(feeLimit *rpc_pb.FeeLimit) {
		queryRoutesReq := &rpc_pb.QueryRoutesRequest{
			PubKey:   dave.PubKey[:],
			Amt:      paymentAmt,
			FeeLimit: feeLimit,
		}
		...
		invoice := &rpc_pb.Invoice{Value: paymentAmt}
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	...
	feeLimitPercent := &rpc_pb.FeeLimit{
		Limit: &rpc_pb.FeeLimit_Percent{
...
func testSendUpdateDisableChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Give Eve some coins.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), eve)
	if err != nil {
...
func testAbandonChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	listReq := &rpc_pb.ListChannelsRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	aliceChannelList, errr := net.Alice.ListChannels(ctxt, listReq)
	if errr != nil {
		t.Fatalf("unable to fetch alice's channels: %v", errr)
	}
	var chanID uint64
	...
	// later state.
	bkupBefore, errr := ioutil.ReadFile(net.Alice.ChanBackupPath())
	if errr != nil {
	...
	abandonChannelRequest := &rpc_pb.AbandonChannelRequest{
		ChannelPoint: chanPoint,
	}

	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	_, errr = net.Alice.AbandonChannel(ctxt, abandonChannelRequest)
	if errr != nil {
		t.Fatalf("unable to abandon channel: %v", errr)
	}
	...
	aliceChannelList, errr = net.Alice.ListChannels(ctxt, listReq)
	if errr != nil {
		t.Fatalf("unable to list channels: %v", errr)
	}
	...
	pendingReq := &rpc_pb.PendingChannelsRequest{}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	alicePendingList, errr := net.Alice.PendingChannels(ctxt, pendingReq)
	if errr != nil {
		t.Fatalf("unable to list pending channels: %v", errr)
	}
	...
	closedReq := &rpc_pb.ClosedChannelsRequest{
		Abandoned: true,
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	aliceClosedList, errr := net.Alice.ClosedChannels(ctxt, closedReq)
	if errr != nil {
		t.Fatalf("unable to list closed channels: %v", errr)
	}
	...
	_, errr = net.Alice.GetChanInfo(ctxb, &rpc_pb.ChanInfoRequest{
		ChanId: chanID,
	})
	if !strings.Contains(errr.Error(), "marked as zombie") {
	...
	_, errr = net.Alice.AbandonChannel(ctxt, abandonChannelRequest)
	if errr != nil {
		t.Fatalf("unable to abandon channel a second time: %v", errr)
	}
...
func testSweepAllCoins(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, btcutil.UnitsPerCoin(), ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoinsNP2WKH(ctxt, btcutil.UnitsPerCoin(), ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}

	// Ensure that we can't send coins to our own Pubkey.
	info, errr := ainz.GetInfo(ctxt, &rpc_pb.GetInfoRequest{})
	if errr != nil {
		t.Fatalf("unable to get node info: %v", errr)
	}

	// Create a label that we will used to label the transaction with.
	sendCoinsLabel := "send all coins"

	sweepReq := &rpc_pb.SendCoinsRequest{
	...
	info, errr = net.Alice.GetInfo(ctxt, &rpc_pb.GetInfoRequest{})
	if errr != nil {
		t.Fatalf("unable to get node info: %v", errr)
	}
	...
	_, errr = ainz.SendCoins(ctxt, sweepReq)
	if errr != nil {
		t.Fatalf("unable to sweep coins: %v", errr)
	}
	...
	_, errr = ainz.WalletKitClient.LabelTransaction(
		ctxt, &walletrpc_pb.LabelTransactionRequest{
			Txid:      sweepHash[:],
			Label:     "",
			Overwrite: false,
		},
	...
	if !strings.Contains(errr.Error(), errZeroLabel) {
	...
	// Our error will be wrapped in a rpc error, so we check that it
	// contains the error we expect.
	if !strings.Contains(errr.Error(), "ErrTxLabelExists") {
	...
	if errr != nil {
		t.Fatalf("unable to get ainz's balance: %v", errr)
	}
	switch {
...
func testHoldInvoicePersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	var (
		payAmt         = btcutil.Amount(4)
		payReqs        []string
		invoiceStreams []invoicesrpc_pb.Invoices_SubscribeSingleInvoiceClient
	)
	...
	for _, payStream := range paymentStreams {
		...
		if payment.Status != rpc_pb.Payment_IN_FLIGHT {
	...
	err = wait.NoError(func() er.R {
		...
		paymentsResp, err := net.Alice.ListPayments(ctxt, req)
		if err != nil {
			return er.Errorf("error when obtaining payments: %v",
				err)
		}
		...
		for _, payment := range paymentsResp.Payments {
			...
			if len(payment.Htlcs) == 0 {
				return er.Errorf("no attempt recorded")
			}
		...
		if len(payHashes) != 0 {
			return er.Errorf("payhash not found in response")
		}
	...
	for _, stream := range invoiceStreams {
		...
		if invoice.State != rpc_pb.Invoice_ACCEPTED {
	...
	var (
		statusUpdates []chan *rpc_pb.Payment
		wg            sync.WaitGroup
		quit          = make(chan struct{})
	)
	...
	for _, preimg := range preimages {
		...
		payStream, err := net.Alice.RouterClient.TrackPaymentV2(
			ctx, &routerrpc_pb.TrackPaymentRequest{
				PaymentHash: hash[:],
			},
		)
		if err != nil {
	...
	for i, preimage := range preimages {
		...
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
		if i%2 == 0 {
			settle := &invoicesrpc_pb.SettleInvoiceMsg{
				Preimage: preimage[:],
			}
			_, errr = carol.SettleInvoice(ctxt, settle)
		} else {
			hash := preimage.Hash()
			settle := &invoicesrpc_pb.CancelInvoiceMsg{
				PaymentHash: hash[:],
			}
			_, errr = carol.CancelInvoice(ctxt, settle)
		}
		if errr != nil {
			t.Fatalf("unable to cancel/settle invoice: %v", errr)
		}
	}
	...
	for i, upd := range statusUpdates {
		...
		for payment == nil {
			...
			case p, ok := <-upd:
				...
				if p.Status == rpc_pb.Payment_IN_FLIGHT {
					continue
				}
		...
		if i%2 == 0 {
			if payment.Status != rpc_pb.Payment_SUCCEEDED {
				t.Fatalf("state not succeeded : %v",
					payment.Status)
			}
		} else {
			if payment.FailureReason !=
				rpc_pb.PaymentFailureReason_FAILURE_REASON_INCORRECT_PAYMENT_DETAILS {
	...
	ctxt, _ = context.WithTimeout(ctxt, defaultTimeout)
	paymentsResp, errr := net.Alice.ListPayments(ctxt, req)
	if errr != nil {
		t.Fatalf("error when obtaining Alice payments: %v", errr)
	}
	for i, preimage := range preimages {
...
func testExternalFundingChanPoint(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	_, errr = carol.AbandonChannel(ctxt, &rpc_pb.AbandonChannelRequest{
		ChannelPoint:           chanPoint1,
		PendingFundingShimOnly: true,
	})
	require.NoError(t.t, errr)
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	_, errr = dave.AbandonChannel(ctxt, &rpc_pb.AbandonChannelRequest{
		ChannelPoint:           chanPoint1,
		PendingFundingShimOnly: true,
	})
	require.NoError(t.t, errr)
...
func deriveFundingShim(net *lntest.NetworkHarness, t *harnessTest,
	...
	thawHeight uint32, keyIndex int32, publish bool) (*rpc_pb.FundingShim,
	*rpc_pb.ChannelPoint, *chainhash.Hash) {
	...
	carolFundingKey, errr := carol.WalletKitClient.DeriveKey(ctxb, keyLoc)
	require.NoError(t.t, errr)
	daveFundingKey, errr := dave.WalletKitClient.DeriveKey(ctxb, keyLoc)
	require.NoError(t.t, errr)
	...
	chanPointShim := &rpc_pb.ChanPointShim{
		Amt:       int64(chanSize),
		ChanPoint: chanPoint,
		LocalKey: &rpc_pb.KeyDescriptor{
			RawKeyBytes: daveFundingKey.RawKeyBytes,
			KeyLoc: &rpc_pb.KeyLocator{
	...
	fundingShim := &rpc_pb.FundingShim{
		Shim: &rpc_pb.FundingShim_ChanPointShim{
			ChanPointShim: chanPointShim,
		},
	}
	_, errr = dave.FundingStateStep(ctxb, &rpc_pb.FundingTransitionMsg{
		Trigger: &rpc_pb.FundingTransitionMsg_ShimRegister{
	...
	fundingShim.GetChanPointShim().LocalKey = &rpc_pb.KeyDescriptor{
...
func sendAndAssertSuccess(t *harnessTest, node *lntest.HarnessNode,
	req *routerrpc_pb.SendPaymentRequest) *rpc_pb.Payment {

	ctx, cancel := context.WithTimeout(context.Background(), defaultTimeout)
	defer cancel()

	var result *rpc_pb.Payment
	err := wait.NoError(func() er.R {
		stream, errr := node.RouterClient.SendPaymentV2(ctx, req)
		if errr != nil {
			return er.Errorf("unable to send payment: %v", errr)
		}

		var err er.R
		result, err = getPaymentResult(stream)
		if err != nil {
			return er.Errorf("unable to get payment result: %v",
				err)
		}

		if result.Status != rpc_pb.Payment_SUCCEEDED {
			return er.Errorf("payment failed: %v", result.Status)
		}
...
func sendAndAssertFailure(t *harnessTest, node *lntest.HarnessNode,
	req *routerrpc_pb.SendPaymentRequest,
	failureReason rpc_pb.PaymentFailureReason) *rpc_pb.Payment {
	...
	if result.Status != rpc_pb.Payment_FAILED {
...
func getPaymentResult(stream routerrpc_pb.Router_SendPaymentV2Client) (
	*rpc_pb.Payment, er.R) {
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	util.RequireNoErr(t, err, "failed to create new miner")
	defer func() {
		util.RequireNoErr(t, minerCleanUp(), "failed to clean up miner")
	}()
	...
	for idx, testCase := range testCases {
		...
		success := t.Run(name, func(t1 *testing.T) {
			...
			util.RequireNoErr(t1,
				err, "unable to set up test lightning network",
			)
			defer func() {
				util.RequireNoErr(t1, lndHarness.TearDown())
			}()
			...
			util.RequireNoErr(t1, err, "unable to add to log")
